...
public class InterfacciaFerie {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InterfacciaFerie window = new InterfacciaFerie();
	...
	private void initialize() {
		...
		frame.setTitle("Richiesta Ferie");
		frame.getContentPane().setLayout(null);
		
		JLabel lblNewLabel = new JLabel("Inserire la data di inizio ferie.");
		lblNewLabel.setBounds(113, 10, 199, 13);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (date == null) {
					...
					boolean verifica = GestoreRichiestaFerie.creaFerie(date, date2);	
					if (verifica) {
						frame.dispose();
					}	
				}
